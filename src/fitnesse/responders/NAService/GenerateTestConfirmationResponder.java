...
public class GenerateTestConfirmationResponder implements SecureResponder {
  ...
  public Response makeResponse(FitNesseContext context, Request request) throws Exception {
    ...
    response.setLastModifiedHeader("Generate");
    return response;
  }

  private String makeDirectoryListingPage(String filename, FitNesseContext context) throws Exception {
    HtmlPage page = context.htmlPageFactory.newPage();
    page.title.use("Generate Test File(s): ");
    page.header.use(HtmlUtil.makeBreadCrumbsWithPageType(resource + filename, "/", "Generate Test File"));
  ...
  private String messageText(String filename, boolean dir, File file) {
    String message = "Are you sure you would like to generate test of <b>" + filename + "</b> ";
    if (dir)
      message += " and all " + file.listFiles().length + " files inside";

    return message + "?" + " If previously exists it will be removed.";
  }
  ...
  private HtmlTag makeYesForm(String filename) {
    ...
    yesForm.add(HtmlUtil.makeInputTag("hidden", "responder", "GenerateTestNAService"));
